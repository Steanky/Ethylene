 ...
 * references whose lifetime exceeds that of the owner class.<p>
 * <p>
 * In effect, this results in the lifetime of bound types being scoped to the lifetime of the classloader that created
 * the owner class. Assuming care is taken to avoid strong static references, this can prevent memory leaks in certain
 * situations where there are multiple classloaders.<p>
 * <p>
 ...
 * holds a strong static reference to a {@code MappingConfigProcessor}. This processor contains a
 ...
 * classes loaded by classloader B can ever be garbage collected unless classloader A is. Since classes hold a reference
 * to their classloader, the signature (which is indirectly statically referenced by a class in classloader A, meaning
 * it will not be garbage collected until the classloader is) is single-handedly keeping classloader B loaded.<p>
 * <p>
 ...
 * classes is statically referenced under the classloader used to load Ethylene.<p>
 * <p>
 ...
 * ones actually held in the map).<p>
 */
