            ...
            Reference reference = GraphTransformer.process(rootEntry, classEntry -> {
                ...
                return new GraphTransformer.Node<>(classEntry, () -> new Iterator<>() {
                    ...
                    public void accept(Object key, Reference value) {
                        ...
                        if (i == args.length) {
                            classEntry.reference.ref = classEntry.typeFactory.make(signature, args);
                        }
                    }
                }));
            }, potentialContainer -> {
                ...
                if (!hint.compatible(potentialContainer.configElement)) {
                    throw new MapperException("mismatched types " + hint + " and " + potentialContainer.type.getTypeName());
                }
